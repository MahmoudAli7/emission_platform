...
Persist the raw measurements.
Atomically update the site’s total_emissions_to_date summary.
...
Quick Start
Initialize environment: cp .env.example .env
Start infrastructure: docker compose up -d
(Optional) DB Admin UI: docker compose --profile tools up -d (available at port 5050).
